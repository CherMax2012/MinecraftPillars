**RU:**

**Как установить версию?**
Это не просто карта, а готовая сборка Minecraft с установленными модами, мирами и датапаками.
1. Скачайте архив с версией
2. Распакуйте архив по пути %AppData%\.minecraft\versions (это стандартное расположение папки с игрой, путь может отличаться)
3. Запустите сборку Minecraft с помощью лаунчера предварительно выбрав именно эту сборку.

**Как играть по сети?**
1. Установите версию на все компьютеры, с которых нужно зайти в мир
2. Откройте мир для сети (в локальной сети или через Radmin VPN)
3. Зайдите в локальный мир
(Если мир не появляется в списке миров сетевой игры, нажмите кнопку "По адресу" и введите IP адрес хоста и порт через двоеточие)

Все действия были выполнены в TLauncher

_______________________________________________

**EN:**

**How ​​do I install this version?**
This isn't just a map, but a complete Minecraft build with mods, worlds, and datapacks installed.
1. Download the version archive
2. Unzip the archive to %AppData%\.minecraft\versions (this is the default location of the game folder; the path may vary)
3. Launch the Minecraft build using the launcher, selecting this build first.

**How ​​to play online?**
1. Install the version on all computers you need to access the world from.
2. Open the world online (via a local network or via Radmin VPN).
3. Join the local world.
(If the world doesn't appear in the list of online worlds, click the "By Address" button and enter the host's IP address and port separated by a colon.)

All steps were performed in TLauncher
